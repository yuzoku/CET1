...
int KeyMngsvr_DBOp_GetKeyID(void *dbhdl, int *keyid)
{
	int 			ret = 0;
	int 			ikeysn;
	...
	{
		printf("param error xxxxxxxxxxxxxxxxx\n");
		ret = -1;
		ITCAST_LOG(__FILE__, __LINE__,LogLevel[4], ret,"func KeyMngsvr_DBOp_GenKeyID() err");
       	goto END;
	}
	...
	if (ret != 0)
	{
    	...
    	ITCAST_LOG(__FILE__, __LINE__,LogLevel[4], ret,"func IC_DBApi_ExecSelSql() err,\n sql:%s", mysql1);
    	goto END;	
	}
    ...
    if (ret != 0)
    {
		printf("IC_DBApi_ExecNSelSql error.......\n");
    	ITCAST_LOG(__FILE__, __LINE__,LogLevel[4], ret,"func IC_DBApi_ExecNSelSql() err, sql:%s", mysql2);
    	goto END;
    }
...
int KeyMngsvr_DBOp_WriteSecKey(void *dbhdl, NodeSHMInfo *pNodeInfo) 
{
    ...
    if (ret != 0)
    {
		printf("IC_DBApi_GetDBTime error...\n");
    	ITCAST_LOG(__FILE__, __LINE__,LogLevel[4], ret,"func IC_DBApi_GetDBTime() err");
       	goto END;
    }
    ...
    if (ret != 0)
    {
		printf("IC_DBApi_Der2Pem error...\n");
    	ITCAST_LOG(__FILE__, __LINE__,LogLevel[4], ret,"func IC_DBApi_Der2Pem() err\n");
       	goto END;
    }
    ...
    {
		printf("IC_DBApi_ExecNSelSql error...\n");
    	ITCAST_LOG(__FILE__, __LINE__,LogLevel[4], ret, "func IC_DBApi_ExecNSelSql() err \n sql===>%s\n", mysql);
        goto END;
    }
