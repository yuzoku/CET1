	...
	ITCAST_UINT32 **ppTagSize)
{
	...
	if (pMidValue == NULL)
	{
		ITCAST_LOG(__FILE__, __LINE__, LogLevel[4], ITDER_MemoryErr, "func DER_ItAsn1_Low_GetTagInfo() err");
		...
		if (iLength <= 0xFF)
			...
			if (iLength < 0xFFFF)
				...
				if (iLength <= 0xFFFFFF)
					...
					{
						ITCAST_LOG(__FILE__, __LINE__, LogLevel[4], ITDER_LengthErr, "func DER_ItAsn1_Low_Count_LengthOfSize() err");
	...
	ITCAST_UINT32 **ppLengthSize)
{
	...
	else                                //long
	{
		...
		if (cSizeOf > 4/* ||cSizeOf <0*/)
		{
			...
			ITCAST_LOG(__FILE__, __LINE__, LogLevel[4], ITDER_LengthErr, "func DER_ItAsn1_Low_GetLengthInfo() err");
			return ITDER_LengthErr;
		}
		else
		{
	...
	ITCAST_UINT8 **ppUint8Value)
{
	...
	{
		ITCAST_LOG(__FILE__, __LINE__, LogLevel[4], ITDER_LengthErr, "func DER_ItAsn1_Low_WriteTagAndLength() err");
		return ITDER_LengthErr;
	}
	...
	ITCAST_UINT8 **ppUint8Value)
{
	...
	if (iMidLength != pAnyIn->dataLen)
	{
		...
		ITCAST_LOG(__FILE__, __LINE__, LogLevel[4], ITDER_LengthNotEqualErr, "func DER_ItAsn1_Low_ReadTagAndLength() err");
	...
	ITASN1_INTEGER **ppDerInteger)
{
	...
	{
		ITCAST_LOG(__FILE__, __LINE__, LogLevel[4], iResult, "func DER_ItAsn1_Low_IntToChar() err != ITDER_NoErr");
		return  iResult;
	}
	...
	ITCAST_UINT32 *pInteger)
{
	...
	if (iResult != ITDER_NoErr)
	{
		ITCAST_LOG(__FILE__, __LINE__, LogLevel[4], iResult, "func DER_ItAsn1_ReadInteger() err check iResult != ITDER_NoErr");
		return  iResult;
	}
	...
	if (iResult != ITDER_NoErr)
	{
		ITCAST_LOG(__FILE__, __LINE__, LogLevel[4], iResult, "func DER_ItAsn1_Low_GetLengthInfo() err");
		return  iResult;
	}
	lMidLength = *pMidSize;
	DER_ITCAST_Free(pMidSize);
	...
	if (iResult != ITDER_NoErr)
	{
		ITCAST_LOG(__FILE__, __LINE__, LogLevel[4], iResult, "func DER_ItAsn1_Low_CharToInt() err");
		return  iResult;
	}
	//输出
	*pInteger = *pMidSize;
	...
	ITASN1_BITSTRING **ppDerBitString)
{
	...
	if (iResult != ITDER_NoErr)
	{
		ITCAST_LOG(__FILE__, __LINE__, LogLevel[4], iResult, "func DER_ItAsn1_Low_WriteTagAndLength() err");
		return  iResult;
	}
	...
	ITASN1_BITSTRING **ppBitString)
{
	...
	if (iResult != ITDER_NoErr)
	{
		ITCAST_LOG(__FILE__, __LINE__, LogLevel[4], iResult, "func DER_ItAsn1_Low_ReadTagAndLength() err");
		return iResult;
	}
	...
	ITASN1_PRINTABLESTRING **ppDerPrintString)
{
	...
	if (pPrintString->dataType == ITCAST_DER_STRING_BMP)
	{
		...
		if (iResult != ITDER_NoErr)
		{
			ITCAST_LOG(__FILE__, __LINE__, LogLevel[4], iResult, "func DER_ItAsn1_WriteBmpString() err");
			return  iResult;
		}
	...
	else
	{
		...
		if (iResult != ITDER_NoErr)
		{
			ITCAST_LOG(__FILE__, __LINE__, LogLevel[4], iResult, "func DER_ItAsn1_WriteCharString() err");
			return  iResult;
		}
	}
	...
	ITASN1_SEQUENCE **ppSequence)
{
	...
	if (iResult != ITDER_NoErr)
	{
		ITCAST_LOG(__FILE__, __LINE__, LogLevel[4], iResult, "func DER_ItAsn1_Low_GetTagInfo() err");
		return  iResult;
	}
	...
	if ((ITCAST_UINT8)*pTagValue != ITCAST_DER_ID_SEQUENCE)
	{
		...
		ITCAST_LOG(__FILE__, __LINE__, LogLevel[4], ITDER_InvalidTagErr, "check pTagValue != ITCAST_DER_ID_SEQUENCE err");
	...
	if (iResult != ITDER_NoErr)
	{
		DER_ITCAST_Free(pTagValue);
		DER_ITCAST_Free(pTagSize);
		ITCAST_LOG(__FILE__, __LINE__, LogLevel[4], iResult, "fun DER_ItAsn1_Low_GetLengthInfo() err");
		return iResult;
	}
	...
	if (pDerSequence->dataLen != *pTagSize + *pLengthSize + *pLengthValue)
	{
		...
		DER_ITCAST_Free(pTagValue);
		DER_ITCAST_Free(pTagSize);
		ITCAST_LOG(__FILE__, __LINE__, LogLevel[4], ITDER_LengthNotEqualErr, "fun check length  err");
		return ITDER_LengthNotEqualErr;
	}
	...
	while (lTotalLength > 0)
	{
		...
		if (iResult != ITDER_NoErr)
		{
			DER_ITCAST_Free(pTagValue);
			DER_ITCAST_Free(pTagSize);
			ITCAST_LOG(__FILE__, __LINE__, LogLevel[4], iResult, "fun DER_ItAsn1_Low_GetTagInfo() err");
			return iResult;
		}
		...
		if (pMidValue == NULL)
		{
			...
			DER_ITCAST_Free(pTagValue);
			DER_ITCAST_Free(pTagSize);
			ITCAST_LOG(__FILE__, __LINE__, LogLevel[4], ITDER_MemoryErr, "pMidValue is null err");
			return ITDER_MemoryErr;
		}
	...
	if (lTotalLength != 0)
	{
		ITCAST_LOG(__FILE__, __LINE__, LogLevel[4], ITDER_LengthErr, "lTotalLength != 0 err");
	...
	int  strOriginLen)
{
	...
	if (pValueBuf == NULL)
	{
		ITCAST_LOG(__FILE__, __LINE__, LogLevel[4], ITDER_MemoryErr, "malloc err");
		return ITDER_MemoryErr;
	}
	...
	else
	{
		...
		if (pValueBuf->pData == NULL)
		{
			...
			ITCAST_LOG(__FILE__, __LINE__, LogLevel[4], ITDER_LengthErr, "malloc err");
			return ITDER_MemoryErr;
		}
...
int WriteNullSequence(ITCAST_ANYBUF **pOutData)
{
	...
	if (rv != 0)
	{
		...
		ITCAST_LOG(__FILE__, __LINE__, LogLevel[4], rv, "func DER_ItAsn1_WriteNull() err");
		return rv;
	}
	...
	if (rv != 0)
	{
		...
		ITCAST_LOG(__FILE__, __LINE__, LogLevel[4], rv, "func DER_ItAsn1_WriteSequence() err");
		return rv;
	}
	DER_ITCAST_FreeQueue(pTmp);
	if (pHead == NULL)
	{
		ITCAST_LOG(__FILE__, __LINE__, LogLevel[4], -1, " check (pHead == NULL) err");
		return -1;
	}
	*pOutData = pHead;
	return 0;
